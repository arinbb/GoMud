...
const questStartSubjects    = ["quest", "desire", "help", "gift"];
...
function onAsk(mob, room, eventDetails) {
    ...
    if ( user.HasQuest(QUEST_END_ID) ) {
        mob.Command("say Thanks for the gift. You're good to go in.");
        return true;
    }
    ...
    if ( !user.HasQuest(QUEST_START_ID) ) {
        ...
        if ( match.found ) {
            ...
            mob.Command("To dance * dance * dance in the underground club, you must prepare a gift!");
            //
    ...
    if ( match.found ) {
        ...
        mob.Command("say Look around, you'll find something!");
        return true;
    }
...
function onIdle(mob, room) {
    ...
    if ( UtilGetRoundNumber() % 4 == 0 ) {
        ...
        mob.Command('say The perfect gift will get you in. <ansi fg="command">Ask</ansi> me about it.');
